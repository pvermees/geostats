...
\noindent\begin{minipage}[t][][b]{.3\textwidth}
  \includegraphics[width=\textwidth]{../figures/qqfaithful1.pdf}\\
\end{minipage}
...
\noindent\begin{minipage}[t][][b]{.3\textwidth}
  \includegraphics[width=\textwidth]{../figures/qqfaithful2.pdf}\\
\end{minipage}
\begin{minipage}[t][][t]{.7\textwidth}
...
\noindent\begin{minipage}[t][][b]{.3\textwidth}
  \includegraphics[width=\textwidth]{../figures/qqfaithful10.pdf}\\
\end{minipage}
\begin{minipage}[t][][t]{.7\textwidth}
...
\end{minipage}
\begin{minipage}[t][][t]{.7\textwidth}
  \captionof{figure}{ Q-Q plot comparing datasets $X_1$ and $X_2$ of
...
The comparison between a sample (coin densities) and a particular
value (19.30~g/cm\textsuperscript{3}) is called a \textbf{one sample
  t-test}. If we want to compare the densities of two samples, then
that would require a \textbf{two sample t-test}. For example, consider
the following two collections of coins:
...
The average densities of collection~1 and 2 are 19.275~g/cm$^3$ and
19.325~g/cm$^3$, respectively. If we assume that the two collections
...
\item Calculate the following test statistic:
  ...
  \noindent where $\bar{x}_1 = 19.275$, $n_1 = 5$, $\bar{x}_2 =
  19.325$, $n_2 = 4$, and $s_p$ is the \emph{pooled} standard
  deviation:
...
\begin{minipage}[t][][t]{.4\textwidth}
  \captionof{figure}{a) PDF and b) CDF of a t-distribution with 7
    ...
    $\alpha=0.05$) is marked in black. The vertical dashed line marks
    the observed value of $t=-2.014$ and plots outside the rejection
...
\begin{minipage}[t][][t]{.4\textwidth}
  \captionof{figure}{The grey and black lines mark the PDFs (a) and
    ...
    $\bar{x}$. However this simple procedure only works if sample size
...
predicting the predicted counts.\\
...
\item  Formulate two hypotheses:
  ...
  $H_\circ$ (\textbf{null hypothesis}):
  the zircon data follow a Poisson distribution

  $H_a$ (\textbf{alternative hypothesis}):
  the zircon data do not follow a Poisson distribution
...
\begin{minipage}[t][][t]{.4\textwidth}
  \captionof{figure}{a) PDF and b) CDF of the $\chi^2$-distribution
    with 5 degrees of freedom. The rejection region is marked in
    black.  The observed value for the earthquake data ($\chi^2=5.14$)
...
sample B & 25 & 12 & 10 & 35
  \end{tabular}
  \captionof{table}{Observed clast counts for two sets of cobbles.}
...
sample B & 23.3 & 11.30 & 10.70 & 36.7
  \end{tabular}
  \captionof{table}{Expected clast counts for two sets of cobbles.}
...
\noindent\includegraphics[width=\textwidth]{../figures/cherrypicking.pdf}\\
\begingroup
\captionof{figure}{a) predicted frequency distribution for the zircon
  ...
  unlikely for a sample to fit the prediction so well.\\}
  \label{fig:cherrypicking}
\endgroup

The first sample is identical to the predicted distribution
($\chi^2$-statistic = 0.0, p-value = 1.0).  Such a `good' result would
be extremely unlikely to happen by chance.

\section{Effect size (Type-II errors revisited)}
\label{sec:effectsize}

Let us carry out a similar experiment to
section~\ref{sec:contingency}, but instead of counting a few 37
...
\begin{center}
  ...
  \end{tabular}
  \captionof{table}{Point counting data for two samples of sand.}
...
With a sample size of more than 100,000 counts per sample, it is not
...
\begin{itemize}
  ...
  \item The $\chi^2$-test requires binning the data into a
    ...
    distribution. Each bin in the histogram requires a sufficient
    number of counts for the $\chi^2$-assumption to be valid. This
    requirement may not be fulfilled for small samples.
\end{itemize}
...
\item Under $H_\circ$, the test statistic follows a Wilcoxon rank-sum
  ...
  \begin{center}
    \begin{tabular}{c|c@{\gap}c@{\gap}c@{\gap}c@{\gap}
      ...
      0.25 & 0.1 & \emph{0.05} & 0.025 & 0.010
    \end{tabular}
  \end{center}
...
\item Marking the two-sided rejection region in bold:
  ...
  \begin{center}
    \begin{tabular}{c|c@{\gap}c@{\gap}c@{\gap}c@{\gap}
      ...
      0.5 & 0.75 & 0.9 & \emph{0.95} & 0.975 & 0.99 \\
      $P(w\geq{W})$ & 0.99 & 0.975 & 0.95 & 0.9 & 0.75 & 0.5 &
      0.25 & 0.1 & \emph{0.05} & 0.025 & \textbf{0.010}
    \end{tabular}
  \end{center}

  The rejection region consists of all $W\leq{11}$ and $W\geq{27}$.
...
The \textbf{Kolmogorov-Smirnov test} is non-parametric alternative to
...
\item Marking the one-sided rejection region in bold:
  ...
  The rejection region consists of all $D\leq{0.174}$.
...
\noindent\begin{minipage}[t][][b]{.6\textwidth}
  \includegraphics[width=\textwidth]{../figures/KSdens.pdf}\\
\end{minipage}
\begin{minipage}[t][][t]{.4\textwidth}
...
\noindent\begin{minipage}[t][][b]{.35\textwidth}
  \includegraphics[width=\textwidth]{../figures/KSnorm.pdf}\\
\end{minipage}
\begin{minipage}[t][][t]{.65\textwidth}
