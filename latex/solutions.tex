...
\item\label{it:xyrandsol} Even though \texttt{runif($\ldots$)} only
  produces values from 0 to 1, it is easy to map these values to any
  other interval:

\begin{script}
library(MASS)
x <- runif(1000)*2 - 1  # generates values from -1 to +1
y <- runif(1000)*20 + 2 # generates values from 2 to 22
contour(kde2d(x,y))
points(x,y)
\end{script}
...
\item To answer the first questions, we formulate a two-sided
  hypothesis:
...
CLT <- function(np=100,ns=200){
  r <- rpois(n=np*ns,lambda=1.2)   # create all the random numbers at once
...
pendulum(startpos=c(-1.9,2))        # slightly different starting position
...
Adding the geometric mean as a white filled square:

\begin{script}[firstnumber=6]
geometric.mean <- exp(colMeans(log(alkali)))
ternary(geometric.mean,add=TRUE,type='p',pch=22,bg='white')
\end{script}
