...
Let us continue with our gold prospecting example. Given that only one
of the five prospectors found gold, our best estime for the abundance
of gold-bearing claims in the prospecting area is $\hat{p} = 2/5$
(40\%). However the introductory paragraph to this chapter mentioned
that geological evidence suggests that 2/3 (67\%) of the claims should
contain gold. Can the discrepancy between the predicted and the
observed number of successes by attributed to bad luck, or does it
mean that the geological estimates were wrong? To answer this
...
test, because $H_\circ$ and $H_a$ are asymmetric. Displaying the
rejection region graphically:\\
...
\begin{minipage}[t][][t]{.4\textwidth}
  \captionof{figure}{a) PMF and b) CDF of a binomial null distribution
    ...
    for $k=2$, which is greater than 0.05. Therefore, the one-sided
    null hypothesis cannot be rejected.}
  \label{fig:1sidedbinomialrejection5}
\end{minipage}

Alternatively, we can also formulate a \textbf{two-sided} hypothesis
test:
...
\item Again, we fail to reject $H_\circ$, which means that the one
  successful claim does not rule out the possibility that the true
  value of $p=2/3$, and that the geologists were therefore correct.
...
\begin{minipage}[t][][t]{.4\textwidth}
  \captionof{figure}{a) the same PMF and b) CDF as
    ...
    rejection region for a two sided hypothesis test of $H_\circ:
    p=2/3$. Horizontal dotted lines mark the $\alpha/2=0.025$ and
    $(1-\alpha/2)=0.975$ cutoff marks. Their intersections with the
    CDF are shown as two vertical dotted lines. The horizontal dashed
    line marks $P(k\leq{2}|p=2/3)=0.2099$.  This value falls between
    $\alpha/2$ and $(1-\alpha/2)$. Therefore $H_\circ$ cannot be
    rejected.\\}
...
which is the same as before. The one-sided hypothesis test ($H_\circ:
...
The p-value is $(2\times{0.0308}=) 0.0616>\alpha$, and the rejection
...
\begin{minipage}[t][][t]{.4\textwidth}
  \captionof{figure}{a) PMF and b) CDF of the binomial distribution
    ...
    quantiles. The dashed lines mark the observed value ($k=6$,
    vertical) and its cumulative probability (0.0308,
    horizontal). $k=6$ falls outside the rejection region and
    $0.0308>\alpha/2$.  Therefore $H_\circ$ cannot be rejected.}
...
\begin{minipage}[t][][t]{.35\textwidth}
  \captionof{figure}{The PMF (a \& b) and CDF (c \& d) for a binomial
    ...
    regions marked in black.  The p-value of the one-sided test (a \&
    c) is 0.0025 and the p-value of the two-sided test (b \& d) is
    0.005. Both of these values are less than $\alpha$ and therefore
...
\item{\bf The degree to which $H_\circ$ is false}
  ...
  area does contain any gold at all ($p=0$). Then the probability of
  finding gold is obviously zero ($F(x=2)=0$). Under this trivial
  ...
  \end{minipage}
  \begin{minipage}[t][][t]{\textwidth-27pt}
...
\item{\bf Sample size}
  ...
  \begin{minipage}[t][][b]{\textwidth-27pt}
...
not prove that Earth's core consists of perovskite. There are lots of
other minerals that are stable at lower mantle pressures. The only
thing that we can say is that the null hypothesis has survived to live
another day. The scientific method is strikingly similar to the way in
which a statistical hypothesis test is carried out. A null hypothesis,
like a scientific hypothesis, cannot be proven. It can only be
disproved. Rejection of a null hypothesis is the best outcome, because
it is the only outcome that teaches us something new.\\
...
maximum likelihood remains $\hat{p}=2/5$ as in our first example, but
the 95\% confidence interval narrows down to
