...
\item{\bf The degree to which $H_0$ is false}
  ...
  \captionof{figure}{a) -- c) PMFs of the binomial null distribution
    with $p=2/3$ ($H_0$, black and grey, bottom) and alternative
    distributions ($H_{\!A}$, black and white, top) with a) $p=2/5$,
    b) $p=1/5$ and c) $p=0$. Sample size is $n=5$ for all cases.  d)
    -- f) CDFs for the null distribution (grey) and the alternative
    ...
    test ($1-\beta$).  Power increases as the alternative distribution
    drifts away from the null distribution.}
...
\item{\bf Sample size}
  ...
  \begin{minipage}[t][][t]{\linewidth}
    \captionof{figure}{a) -- c) PMFs of binomial distributions with
      $p=2/3$ ($H_0$, black and grey, bottom) and $p=2/5$ ($H_{\!A}$,
      black and white, top), for sample sizes of a) $n=5$, b) $n=15$
      ...
      ($1-\beta$).  Power increases with sample size.}
