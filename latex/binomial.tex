...
\item Mark all the outcomes that are incompatible with $H_0$,
...
\begin{minipage}[t][][t]{.4\textwidth}
  \captionof{figure}{a) the same PMF and b) CDF as
    ...
    rejection region for $H_0: p=2/3$ vs $H_{\!A}:
    ...
    which fall outside the rejection region. Therefore $H_0$
    stands.}
...
\noindent which includes $k=6$. Therefore $H_0$ has been rejected.
...
\begin{minipage}[t][][t]{.4\textwidth}
  \captionof{figure}{a) PMF and b) CDF of the binomial distribution
    ...
    falls outside the rejection region.  Therefore $H_0$ stands.}
...
\begin{enumerate}
  ...
  \item Increasing the number of prospectors to $n=15$ leads to a
    rejection of $H_0$ in the one-sided case but failure to reject
    $H_0$ in the two-sided test.
...
\item{\bf The degree to which $H_0$ is false}
  ...
  rejection region for the one-sided hypothesis test of $H_0:
  p=2/3$ vs. $H_{\!A}: p<2/3$ is $R=\{0,1\}$. If the actual value for $p$
  ...
  \captionof{figure}{a) -- c) PMFs of the binomial null distribution
    ...
    is to reject $H_0$. The dashed horizontal lines mark the
...
\item{\bf Sample size}
  ...
  null hypothesis $H_0: p=2/3$ to those of the alternative
  hypothesis $H_{\!A}: p=2/5$ for sample sizes of $n=5$, 15 and 30:
  ...
  \begin{minipage}[t][][t]{\linewidth}
    \captionof{figure}{a) -- c) PMFs of binomial distributions with
      ...
      size, the easier it is to reject $H_0$. The dashed
