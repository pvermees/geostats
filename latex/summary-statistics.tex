...
Calculating the same three summary statistics for the catchment area
data:

\noindent\begin{minipage}[t][][b]{.5\textwidth}
\includegraphics[width=\textwidth]{../figures/arealocation.pdf}
\end{minipage}
\begin{minipage}[t][][t]{.5\textwidth}
  \captionof{figure}{KDE and ECDF of the catchment area data (mean =
    3.2, median = 2.1, mode = 0.45). The mean is strongly affected by
    the long `tail' of large outliers. It is 7 times greater than the
    mode. Only 6 out of 20 catchments (30\%) are larger than the mean
    of the distribution and only 1 (5\%) is smaller than the mode.}
  \label{fig:arealocation}
\end{minipage}
...
pH and catchment area datasets alongside the distribution of Covid-19
...
\begingroup \captionof{figure}{a) the frequency distributions of pH
  ...
  slightly negative) skewness; b) the catchment area measurements are
