...
\begin{minipage}[t][][t]{.65\textwidth}
  \captionof{figure}{Synthetic dataset of bivariate data that belong
    to two classes. The black circles are split into two data
...
\begin{minipage}[t][][t]{.7\textwidth}
  \captionof{figure}{A plot of cross validated (CV) prediction error
    ...
    smallest CV error (dotted line). For the example dataset, this
    supports an optimal tree with four splits.\\}
  \label{fig:cvCART}
\end{minipage}

For our bivariate example, this procedure produces a tree with four
