...
two-dimensional dataset onto a one dimensional line. We can use
another a toy example to illustrate how LDA can achieve the same
effect with a different aim. Consider a mixed dataset of two bivariate
samples, $X$ and $Y$:
\begin{equation}
  X = \bbordermatrix{
    ~ &  a & b \cr
...
The linear discriminant function is obtained by maximising the
difference between the group means relative to the within-group
(co)variance:
\begin{equation}
...
\begin{equation}
  \mbox{LD}' = \frac{\mbox{LD}}{\sqrt{\mbox{LD}^T\,\Sigma_w\,\mbox{LD}}}
  \bbordermatrix{
    ~ & a & b \cr
    = & 0.52 & -0.20
  }
