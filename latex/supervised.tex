...
The outcome of these calculations is shown in
...
\begin{minipage}[t][][t]{.45\textwidth}
  \captionof{figure}{(i) The bivariate toy dataset of
    Equation~\ref{eq:XY} with indication of the first linear
    dicriminant function (LD1) and the first (PC1) and second (PC2)
    principal components. LDA uses the groupings ($X$ and $Y$), PCA
    ignores them. (ii) Projecting the data onto the directions defined
    by LD1, PC1 and PC2 produces three one-dimensional views of the
    toy data. LD1 is the direction that maximises the between-group
...
\begin{minipage}[t][][t]{.65\textwidth}
  \captionof{figure}{The first two linear discriminants of the
    ...
    width - 1.20); and\\
    LD2 = 0.024 $\times$ (sepal length - 5.84) +
