...
of 15 species at 50 sites. This chapter will introduce some tools that
can help us see some structure in such `big' datasets without any
prior knowledge of clusters, groupings or trends in the dataset.  This
is called unsupervised learning, as opposed to the supervised learning
...
\begin{minipage}[t][][t]{.75\textwidth}
  \captionof{figure}{PCA decomposition of
    ...
    as a yellow square, and $1_{2,1}C \pm L$ as a cross. The first
    principal direction (running from the upper left to the lower
    right) has been stretched by a factor of $(3.67/0.71) = 5.2$ w.r.t
    the second principal direction, which runs perpendicular to it.\\}
  \label{fig:PCA2D1}
\end{minipage}
...
\begin{tabular}{l|cccc}
      & Murder & Assault & UrbanPop & Rape \\ \hline
...
and rape are all point in approximately the same direction, towards
the left hand side of the diagram. This tells us that the crimes are
all correlated with each other. So states that have a lot of assaults
...
K-means cluster is an unsupervised learning algorithm that tries to
...
\item Randomly select three data points from the dataset and designate
  ...
  \noindent\begin{minipage}{\textwidth-27pt}
  \noindent\begin{minipage}[t][][b]{.25\textwidth}
...
\item Identify the closest two clusters (corresponding to the boxed
  ...
  \noindent\begin{minipage}{\linewidth-27pt}
