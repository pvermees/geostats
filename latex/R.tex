...
pred <- c(ppois(q=2,lambda=lam),                  # lower tail
          dpois(x=3:9,lambda=lam),                # central segment
          ppois(q=9,lambda=lam,lower.tail=FALSE)) # upper tail
\end{script}

\noindent which add up to 1. Then the $\chi^2$-test proceeds as
follows:

\begin{script}[firstnumber=9]
h <- chisq.test(x=obs,p=pred)
\end{script}
