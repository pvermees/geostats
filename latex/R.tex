...
\item\label{it:1sidedbinomR} Calculate the quantiles of the binomial
  ...
  claims is less than or equal to
...
\item Construct a 95\% confidence envelope for the linear fit:
...
Base \texttt{R} does not include little or no built-in functions for
directional data analysis. So this tutorial will heavily depend on the
...
\noindent where the options \texttt{degrees} accounts for the fact
that the \texttt{striations} data are expressed in degrees.  Change to
...
\item Compute the concentration parameter $\bar{R}$:
...
\begin{script}[firstnumber=2]
stereonet(trd=c(0,0,90,180,270),plg=c(90,10,10,10,10),coneAngle=rep(10,5),
          option=4,degrees=TRUE,add=TRUE,wulff=FALSE)
\end{script}
...
stereonet(trd=meanpalaeomag[1],plg=meanpalaeomag[2],
          option=1degrees=TRUE,add=TRUE,pch=16,col='black')
\end{script}
...
Y <- meuse$y  # Northing
Z <- log(meuse$zinc)
colourplot(X=X,Y=Y,Z=Z,colspec=grey,key.title=title('ln[Zn]'))
\end{script}
...
To fit a linear semivariogram model:

\begin{script}[firstnumber=6]
svm <- semivariogram(x=X,y=Y,z=Z,model='linear')
\end{script}

Other options for the \texttt{model} argument are
\verb|'exponential'| and \verb|'gaussian'|.
...
\item Evaluate the ln[Zn]-values for a grid of locations and plot as a
  contour map:
...
colourplot(x=xi,y=yi,z=zi,colspec=grey,key.title=title('ln[Zn]'))
\end{script}
