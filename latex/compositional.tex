...
\begin{minipage}[t][][t]{.6\textwidth}
  \captionof{figure}{Compositional biplot of the major element
    ...
    compositionally different. In contrast with the interpretation of
    ordinary biplots (e.g., Figure~\ref{fig:USArrests}), one should be
    careful not to interpret the individual rays for a compositional
    biplot. For example, it is not safely conclude that N8 is enriched
    in MnO relative to T8. However, it is safe to say that N8 has a
    higher MnO/Na\textsubscript{2}O-ratio than T8. \medskip}
  \label{fig:majorPCA}
\end{minipage}

With regard to the rays, they key information is contained in the
...
long, so their logratios are dispersed. The link between
...
suggesting that the corresponding subcompositions are
independent:
\begin{center}
\noindent\includegraphics[width=.9\textwidth]{../figures/links.pdf}
\captionof{figure}{a) Box plots of the CaO/MgO-logratio (short link)
  and the K\textsubscript{2}O logratio (long link); b) Collinearity of
  the links between TiO\textsubscript{2}, CaO, MnO and MgO causes
  strong correlation of the samples in ln[MnO/MgO] --
  ln[TiO\textsubscript{2}/CaO] space; c) The link between
  P\textsubscript{2}O\textsubscript{5} and TiO\textsubscript{2} is
  orthogonal to that between between K\textsubscript{2}O and CaO,
  causing independence of the corresponding logratios.
    \medskip}
  \label{fig:links}
\end{center}
