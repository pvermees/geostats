...
For example, calculate the distances between the alr-coordinates of
samples~2 and 3 of Equation~\ref{eq:Xcomp} using $b$ and $c$ as a
common denominator:
\begin{equation*}
  \begin{split}
    d[2,3]_b & =
...
spells trouble for PCA. Recall the equivalence between PCA and MDS,
which was discussed in Section~\ref{sec:MDS}. MDS is based on
...
\emph{Aitchison distances}) are well defined. Subjecting
...
two, the scores of the third component in matrix $P$ are all
zero\footnote{The covariance matrix of $X_c$ is singular and therefore
the third eigenvalue is zero as well.}.  Therefore, all the
information is contained in the first two components. Also note that
the first two principal components of the compositional dataset are
identical to those of the PCA example shown in Section~\ref{sec:PCA}
