...
as Equations~\ref{eq:XMY}--\ref{eq:P} yields:
\begin{align}
  \begin{split}
    X_c & = 1_{3,1}~M + P~Q^{-1} \\
    ~ & = \left[
...
two, the scores of the third component are all zero (and so is the
corresponding eigenvalue). Therefore, all the information is contained
...
\begin{minipage}[t][][t]{.7\textwidth}
  \captionof{figure}{Compositional biplot of the toy data using the
    ...
    biplot tells us that sample~1 is rich in component $b$, whereas
