...
pencil on paper).  Does it identify any outliers?
   ...
   value of 40 is an outlier, because $40-9=31>1.5\times$IQR$=22.5$.
...
particle remains, and that $X_i$ can be modelled as independent
random variables having the same distibution (for
$1\leq{i}\leq{n}$, say). Prove that the logarithm of the resulting
particle sizes follows a normal distribution.

   \ifsol{\itshape
   After $n$ impacts, the particle size has been reduced to:
   \[y_n = X_nX_{n-1}\ldots{X_2}{X_1}y0\]
   Taking logarithms:
...
between blood type and propensity for ulcer?
...
2. Are the data from London and Manchester comparable?
