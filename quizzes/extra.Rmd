...
Table 2 of [Nemchin et
al. (2022)](https://www.sciencedirect.com/science/article/pii/S0016703721007158)
...
download
[here](https://github.com/pvermees/geostats/tree/main/quizzes/Nemchin2022trace.csv).

1. Plot the data on a ternary diagram containing Th, U and Ba. Colour
code the samples by group ('basaltic', 'feldspathic', ...).

1. Visualise the data on a compositional biplot.

Generate a compositional dataset by applying an inverse logratio
transformation to the multivariate normal dataset of exercise
\ref{sec:unsupervised}. Label the resulting 6 components $A$, $B$,
$C$, $D$, $E$ and $F$.

   ```{r}
   cdat <- geostats::alr(dat,inverse=TRUE)
   colnames(cdat) <- c('A','B','C','D','E','F')
   ```

1. Plot the compositional data on two ternary diagrams, grouping
components $\{A,B,C\}$ and $\{D,E,F\}$, respectively.

   ```{r out.width="85%", fig.width=8, fig.height=4}
   par(mfrow=c(1,2))
   ternary(cdat[,1:3],pch=16)
   ternary(cdat[,4:6],pch=16)
   ```

1. Carry out a 'naive' PCA analysis, ignoring the compositional nature
of the transformed dataset. Then repeat the analysis using
compositional PCA. Plot the results of both analyses as biplots.  Do
you note any differences?

   ```{r out.width="90%", fig.width=10, fig.height=5}
   par(mfrow=c(1,2))
   biplot(prcomp(cdat))
   biplot(prcomp(clr(cdat)))
   ```

1. Using the known groups ($X$, $Y$ and $Z$) of the synthetic data,
carry out a 'naive' LDA analysis, ignoring the compositional nature of
the transformed dataset. Do you get any warnings? Then repeat the
analysis using compositional LDA.

   ```{r out.width="85%", fig.width=8, fig.height=4}
   par(mfrow=c(1,2))
   nldat <- data.frame(groups=groups,cdat)
