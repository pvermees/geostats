...
1. Draw 10 sets of random values from each of these 3
   ...
   E <- rbind(c(5,1,2,3,4),
              ...
              c(1,2,3,4,5))
   library(MASS)
   A <- mvrnorm(n=n,mu=muA,Sigma=E)
   B <- mvrnorm(n=n,mu=muB,Sigma=E)
   C <- mvrnorm(n=n,mu=muC,Sigma=E)
   dat <- rbind(A,B,C)
...
results to add text labels to the MDS configuration.
...
Create a new dataset of ${3}\times{10}$ samples, but now label the
rows of your dataset with $A1,\ldots,A10$, $B1,\ldots,B10$, and
$C1,\ldots,C10$. Use the `paste0()` function to create the labels.
```{r}
rownames(dat) <- c(paste0('A',1:n),
                   paste0('B',1:n),
                   paste0('C',1:n))
```
