...
lines at every 20 units of $x$ and $y$. \label{it:FT1}
   ...
   ```{r FT1, out.width="70%", out.height="70%", fig.width=10, fig.height=10}
   randgrid <- function(n=1000,m=0,M=500,bw=20){
      ...
      ng <- length(g)
      # instead of 'matlines' you could also use a for loop of 'lines'
...
graticule contained in the interval from $x=0$ to 100 and from $y=0$
to 100. Plot these numbers as a histogram. \label{it:FT2}

   ```{r FT2, out.width="50%", out.height="50%", fig.width=5, fig.height=5}
...
before drawing conclusions.
   ...
   ```{r FT3, fig.width=10, fig.height=5}
   par(mfcol=c(2,3))
   for (i in 1:3){
      counts <- bingrid(xy=randgrid())
      m <- mean(counts)
      v <- var(counts)
      message('mean=',signif(m,2),', var=',signif(v,2))
   }
   ```
...
_anisotropic_ diffusion.
   ...
   cloud <- function(n=1000,N=500,plot=TRUE,a=1,b=1,alpha=0,...){
      xyf <- matrix(NA,nrow=N,ncol=2)
      for (i in 1:N){
         xyf[i,] <- brown(n=n,a=a,b=b,alpha=alpha)[n,]
      }
      if (plot) plot(xyf,pch=16,xlab='x',ylab='y',...)
      invisible(xyf)
   }
...
1. Consider a bivariate normal distribution with the following
...
2. Predict $z = xy$ and estimate its standard error.

3. Draw $n=1000$ pairs of random numbers from the bivariate normal
...
4. Repeat steps 2 and 3 for $z = x^2 y^3$.
...
documentation of the `ks.test()` function for help.

\ifmulti\clearpage\fi
# Regression \label{sec:regression}

1. Calculate the correlation coefficients of the bivariate normal
distributions of exercise \ref{sec:normal}.2
   ...
   for (i in 1:4){
      for (j in 1:N){
         xyf[j,] <- brown(n=n,a=a[i],b=b[i],alpha=alpha[i])[n,]
      }
      adeg <- alpha[i]*180/pi
      message('a=',a[i],', b=',b[i],', alpha=',signif(adeg,2),'deg')
      r <- cor(xyf[,1],xyf[,2])
      message('r[x,y]=',signif(r,2))
   }
   ```
