...
1. Draw 10 sets of random values from each of these 3
   distributions and store them in a data frame. Name the variables
   $a$, $b$, $c$, $d$ and $e$.
   ...
   ABC <- function(n,muA,muB,muC,S){
      A <- mvrnorm(n=n,mu=muA,Sigma=S)
      B <- mvrnorm(n=n,mu=muB,Sigma=S)
      C <- mvrnorm(n=n,mu=muC,Sigma=S)
      out <- rbind(A,B,C)
      colnames(out) <- c('a','b','c','d','e')
      return(out)
   }
   dat <- ABC(n,muA,muB,muC,S)
   ```
...
```{r}
groups <- c(rep('A',n),rep('B',n),rep('C',n))
```

1. Create a 30-element vector with the names of the known classes
($A$, $B$, $C$) of the ${3}\times{10}$ samples generated in the
...
1. Create a new dataset of ${3}\times{10}$ samples from the
...
previous step. What is the misclassification rate?

   ```{r}
   newdat <- ABC(n,muA,muB,muC,S)
   pred <- predict(ld,newdata=as.data.frame(newdat))
   table(pred$class,groups)
   message(sum(pred$class != groups),' samples were misclassified.')
   ```

1. Build a decision tree using the same training data as before. How
many branches does the default tree have?

   ```{r out.width="40%", fig.width=5, fig.height=5}
   library(rpart)
   tree <- rpart(groups ~ dat, method='class')
   plot(tree)
   text(tree,xpd=NA)
   ```

1. If your tree is too small then this is probably because, by
default, `rpart` expects at least 20 items per branch. You can
...
the `rpart` function.  Using the `iris` dataset to illustrate this:
   ...
   my.control <- rpart.control(minsplit=1)
   tree <- rpart(groups ~ dat, method="class", control=my.control)
   plot(tree)
   text(tree,xpd=NA)
   ```
...
tree with `pure' nodes:
   ...
   ```{r out.width="50%", fig.width=5, fig.height=5}
   my.control <- rpart.control(cp=0,minsplit=1)
   tree.unpruned <- rpart(groups ~ dat, method="class", control=my.control)
