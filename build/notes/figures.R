...
pars(mfrow=c(1,2))
p <- par(mfrow=c(1,2))
hist(pH,breaks=seq(from=3,to=7,by=1),col='white',xlim=c(3,7),main='')
rug(pH)
legend('topleft',legend='a)',bty='n',cex=1.2,adj=c(1,-1),xpd=TRUE)
hist(pH,breaks=seq(from=3,to=7,by=0.5),col='white',xlim=c(3,7),main='')
rug(pH)
legend('topleft',legend='b)',bty='n',cex=1.2,adj=c(1,-1),xpd=TRUE)
dev.off()

cairo(file='../../figures/binpos.pdf',width=5,height=2.5)
pars(mfrow=c(1,2))
hist(pH,breaks=seq(from=3,to=7,by=0.5),xlim=c(3,7),main='',col='white')
rug(pH)
legend('topleft',legend='a)',bty='n',cex=1.2,adj=c(1,-1),xpd=TRUE)
hist(pH,breaks=seq(from=3.25,to=6.75,by=0.5),xlim=c(3,7),main='',col='white')
rug(pH)
legend('topleft',legend='b)',bty='n',cex=1.2,adj=c(1,-1),xpd=TRUE)
dev.off()
...
hist(nquakes,breaks=seq(from=-0.5,to=12.5,by=1),main='',
     xlab='# earthquakes/year',col='white')
dev.off()
...
L <- mean(nquakes)
obs <- c(sum(quaketab[1:2]),quaketab[3:8],sum(quaketab[9:12]))
pred <- c(ppois(2,lambda=L),dpois(3:8,L),ppois(8,L,lower.tail=FALSE))
p <- pred/sum(pred)
o <- obs/sum(obs)
...
hist(quakes$mag,breaks=20,main='',
     xlab='magnitude',xaxt='n',xpd=NA,col='white')
axis(side=1,at=ticks,labels=ticks)
dev.off()

cairo(file='../../figures/recentlogquakes.pdf',width=4,height=2.5)
pars()
hist(log(quakes$mag),breaks=20,main='',col='white',
...
boxplot(cbind(log(Major$CaO)-log(Major$MgO),
              log(Major$K2O)-log(Major$MgO)),
        xlab='',xaxt='n',bty='n',col='white')
...
col[th] <- 'white'
ternary(afm[,-1],type='p',pch=21,bg=col,lwd=0.8,labels=c('F','A','M'),cex=0.6)
dev.off()

cairo(file='../../figures/lrAFM.pdf',width=3,height=3)
pars()
lr <- alr(afm[,-1])
plot(lr,pch=21,bg=col,lwd=0.8,xlab='ln(A/F)',ylab='ln(M/F)',cex=0.6)
dev.off()
...
col[th] <- 'white'
plot(lr,pch=21,lwd=0.8,cex=0.6,bg=col,col='gray50',xlab='ln(A/F)',ylab='ln(A/F)')
...
p <- par(mar=rep(0,4))
ternary(afm[,-1],pch=21,lwd=0.8,bg=col,cex=0.6,col='gray50',labels=c('F','A','M'))
...
cairo(file='../../figures/semivariogramfit.pdf',width=7,height=2)
pars(mfrow=c(1,3),mgp=c(1.2,0.5,0))
...
x <- seq(from=0.001,to=1500,length.out=50)
lsnr <- geostats:::.lsnr(c(0.6,0.1,1000))
sv <- geostats:::semivarmod(x,lsnr=lsnr,model='spherical')
plot(x=x,y=sv,type='l',lwd=3,
     xlab='lag',ylab=expression(gamma(lag)),
     xlim=c(-50,1550),ylim=c(0,0.63),bty='n')
arrows(x0=-50,y0=0,x1=-50,y1=0.1,length=0.05,angle=90,code=3)
arrows(x0=1550,y0=0,x1=1550,y1=0.6,length=0.05,angle=90,code=3)
arrows(x0=0,y0=0.62,x1=1000,y1=0.62,length=0.05,angle=90,code=3)
lines(x=c(-50,0),y=c(0.1,0.1),lty=3)
lines(x=c(-50,1550),y=c(0,0),lty=3)
text(x=-50,y=0.05,labels=expression(nugget~(c[n])),pos=4)
lines(x=c(0,0),y=c(0.1,0.62),lty=3)
lines(x=c(1000,1000),y=c(0.6,0.62),lty=3)
text(x=500,y=0.62,labels=expression(range~(c[r])),pos=3,xpd=NA)
...
cairo(file='../../figures/hills.pdf',width=7,height=2)
pars(mfrow=c(1,3),mgp=c(1.2,0.5,0),mar=c(2.5,2.3,0.7,0.25))
...
cairo(file='../../slides/poisquakes.pdf',width=4,height=4)
...
cairo(file='../../slides/arctan2.pdf',width=4,height=4)
...
text(cos(pi/8)*0.65,sin(pi/8)*0.65,labels='arctan(y/x)',pos=3,xpd=NA)
text(cos(pi/8)*0.65,sin(pi/8)*0.65,labels='= arctan2(y,x)',pos=1,xpd=NA)
text(x3[50]*0.7,y3[50]*0.75,labels='arctan(y/(-x))',pos=3,xpd=NA)
text(x3[50]*0.7,y3[50]*0.75,labels='=arctan(-y/x)',pos=1,xpd=NA)
text(x2[50]*1.15,y2[50]*1.2,labels='arctan2(y,-x)',pos=1,xpd=NA)
